package com.wgc.cmwgc.JT808.util;
...
//	private final Logger log = LoggerFactory.getLogger(getClass());
	...
	public int generateMsgBodyProps(int msgLen, int enctyptionType, boolean isSubPackage, int reversed_14_15) {
		...
		if (msgLen >= 1024)
			Log.w("","The max value of msgLen is 1023, but {} ."+ msgLen);
	...
	public byte[] generateMsgHeader(String phone, int msgType, int msgBodyProps, int flowId)
		...
		try {
			baos = new ByteArrayOutputStream();
			// 1. 消息ID word(16)
			baos.write(bitOperator.integerTo2Bytes(msgType));/*0x0100*/
			// 2. 消息体属性 word(16)
	...
	/**
	 * @param headAndBodyBytes
	 * @return 校验码
	 */
	public byte generateCheckCode(byte[] headAndBodyBytes){
		int a = headAndBodyBytes[0];
		for(int i=0;i<headAndBodyBytes.length;i++){
			if (i<headAndBodyBytes.length-1){
				a =  a^ headAndBodyBytes[i+1];
			}
		}
