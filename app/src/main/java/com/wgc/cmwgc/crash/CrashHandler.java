...
public class CrashHandler implements UncaughtExceptionHandler{
    ...
    private void initWistorm(){
        ...
        errorLog = new WErrorLog(mContext);
    }
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        if(!handleException(ex) && mExceptionHandler!=null){
            ...
            //这里要停止服务 不然第二次启动会不行
            mContext.stopService(new Intent(mContext, HttpService.class));
...
//        postErrorReport(throwable);
    ...
    private void postErrorReport(Throwable ex){
        StringBuffer sb = new StringBuffer();
        for (Map.Entry<String, String> entry : infos.entrySet()) {
            String key = entry.getKey();
            String value = entry.getValue();
...
//        params.put("account", Config.USER_NAME);
        ...
        BootUpReceiver.startService(mContext);

        errorLog.create(params, fields, new OnSuccess() {
            @Override
            protected void onSuccess(String response) {
                Logger.d("提交错误日志返回信息 ：" + response);
            }
        }, new OnFailure() {
            @Override
            protected void onFailure(VolleyError error) {
                Logger.e("提交错误日志返回信息 ：" + error.toString());
            }
        });
    }
